...
def test_compress_data_zstd() -> None:
    ...
    data: bytes = b"hello world"
    compressed_data: bytes = compress_data(data, algorithm='zstd', level=3)
...
def test_compress_data_invalid_level_type() -> None:
    """
    Test the compress_data function with a non-integer compression level.
    """
    # Test case 9: Non-integer compression level
    data: bytes = b"hello world"
    with pytest.raises(TypeError):
        compress_data(data, algorithm='zstd', level="not an integer")  # type: ignore
