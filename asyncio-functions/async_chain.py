...
async def async_chain(functions: List[Callable[[T], Awaitable[R]]], input_value: T) -> R:
    """
    Chain multiple asynchronous functions together.

    Parameters
    ----------
    functions : List[Callable[[T], Awaitable[R]]]
    ...
    # Return the final result after chaining all functions
    return value